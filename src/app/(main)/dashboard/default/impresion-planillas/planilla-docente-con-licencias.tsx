...
interface PlanillaDocenteConLicenciasProps {
  onBack: () => void;
}

export default function PlanillaDocenteConLicencias({ onBack }: PlanillaDocenteConLicenciasProps) {
  const [dni, setDni] = useState("21813052");
  const [datosPersonales, setDatosPersonales] = useState({
    apellido: "JOFRÉ ALLENDE",
    nombre: "Rosana Ivana",
    denominacionCargo: "Maestro de Grado Educación Primaria Jornada Extendida",
    tipoTraslado: "Ordinario",
    establecimiento: 'SEGUNDINO J. NAVARRO "EMER"',
  });

  const [antiguedad, setAntiguedad] = useState({
    suplente: { años: "0", meses: "7", dias: "14" },
    titular: { años: "31", meses: "5", dias: "8" },
    licencias: { años: "0", meses: "5", dias: "0" },
    traslado: { años: "31", meses: "7", dias: "22" },
  });
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <CardTitle>Planilla Docente con Licencias</CardTitle>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-4 items-center gap-3">
            <Label className="text-left">Suplente:</Label>
            <Input
              value={antiguedad.suplente.años}
              onChange={(e) =>
                setAntiguedad({
                  ...antiguedad,
                  suplente: { ...antiguedad.suplente, años: e.target.value },
                })
              }
              className="bg-blue-50 text-center"
            />
            <Input
              value={antiguedad.suplente.meses}
              onChange={(e) =>
                setAntiguedad({
                  ...antiguedad,
                  suplente: { ...antiguedad.suplente, meses: e.target.value },
                })
              }
              className="bg-blue-50 text-center"
            />
            <Input
              value={antiguedad.suplente.dias}
              onChange={(e) =>
                setAntiguedad({
                  ...antiguedad,
                  suplente: { ...antiguedad.suplente, dias: e.target.value },
                })
              }
              className="bg-blue-50 text-center"
            />
