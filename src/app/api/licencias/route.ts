...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!numero_documento || !apellido || !nombre || !fecha_inicio) {
      return NextResponse.json({ error: "DNI, apellido, nombre y fecha de inicio son requeridos" }, { status: 400 });
    }
    ...
    if (fecha_fin) {
      ...
      if (isNaN(fechaFin.getTime())) {
        return NextResponse.json({ error: "Fecha de finalización no válida" }, { status: 400 });
      }
    ...
    try {
      ...
      if (!tipoLicenciaId) {
        ...
        const [tiposExistentes] = await connection.execute("SELECT id_tipo FROM tipos_licencias LIMIT 1");
        ...
        if (Array.isArray(tiposExistentes) && tiposExistentes.length > 0) {
          ...
          for (const tipo of tiposBasicos) {
            await connection.execute("INSERT INTO tipos_licencias (descripcion) VALUES (?)", [tipo]);
          }
