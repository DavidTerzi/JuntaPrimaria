...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      const [existingRows] = await connection.execute("SELECT COUNT(*) as count FROM tipos_licencias");
      ...
      if (count === 0) {
        ...
        for (const tipo of tiposBasicos) {
          await connection.execute("INSERT INTO tipos_licencias (nombre) VALUES (?)", [tipo]);
        }
